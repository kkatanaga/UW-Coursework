...
int main(void) {
    ...
    char input[10] = {};
    int word_total = 1;                 // Keeps track of the number of words in a tree
    int word_value = 0;                 // Contains the value associated with a word in a node
    ...
    while( scanf("%9s", input) && strcmp(input, ".") ) {
        ...
        if (word_value == 0) {
            root = insert_node(input, word_total, root);
            ++word_total;
            printf("%s ", input);
        }
        else {
