...
class ConcentricRing extends RingObjectsKK {
	...
	public ConcentricRing(Color3f colorOne, Color3f colorTwo) {
		...
		Assignment5KK.rotations = new HashMap<String, RotationInterpolator>();	// Initialize to keep track of the incoming rotation behaviors
		...
		BaseShape base = new BaseShape(colorOne);
		winner = base.getSwitch();
		outerRingTG.addChild(base.position_Object());							// Add base shape to the ring such that it inherits the outer ring rotation
...
class RingShape extends RingObjectsKK {												// loads a Ring.obj
	...
	protected void add_Rotation(int rot_rate, Transform3D axis) {
		...
		Assignment5KK.rotations.put(ringName, rot_beh);				// /!\ Added in Assignment 4; adds the RotationInterpolator of the shape to the HashMap /!\
