...
public abstract class Lab9ObjectsKK {
...
//===============/!\ Lab 9 shapes /!\===============
class ButtonObjects {
	private Switch lButtonSwitch;
	private Switch rButtonSwitch;
	private TransformGroup switchTG;
	
	public ButtonObjects() {
		ButtonObject leftOff = new ButtonObject("left", CommonsKK.Green);		// create green left  "FanButton"
		ButtonObject leftOn = new ButtonObject("left", CommonsKK.Red);			// create red   left  "FanButton"
		ButtonObject rightOff = new ButtonObject("right", CommonsKK.Green);		// create green right "FanButton"
		ButtonObject rightOn = new ButtonObject("right", CommonsKK.Red);		// create red   right "FanButton"
		
		switchTG = new TransformGroup();
		lButtonSwitch = create_Switch(leftOff, leftOn);
		rButtonSwitch = create_Switch(rightOff, rightOn);
		switchTG.addChild(lButtonSwitch);
		switchTG.addChild(rButtonSwitch);
	}
	private Switch create_Switch(ButtonObject offObj, ButtonObject onObj) {
		Switch switchGroup = new Switch();
		switchGroup.setCapability(Switch.ALLOW_SWITCH_WRITE);
		switchGroup.addChild(onObj.position_Object());							// attach on "FanButton" to Switch
		switchGroup.addChild(offObj.position_Object());							// attach off "FanButton" to Switch
		switchGroup.setWhichChild(0);
		return switchGroup;
	}
	public TransformGroup getSwitchTG() {
		return switchTG;
	}
	public void setLeftSwitch(int index) {
		lButtonSwitch.setWhichChild(index);
	}
	public void setRightSwitch(int index) {
		rButtonSwitch.setWhichChild(index);;
	}
}
//===========/!\ End of Lab 9 shapes /!\============

//===============/!\ Lab 7 shapes /!\===============
class ButtonObject extends Lab9ObjectsKK {
	public ButtonObject(String loc, Color3f color) {
		...
		mtl_clr[1] = color;       									// set "FanButton" to the input color 		                                              
...
//===============/!\ Lab 6 shapes /!\===============
class ShaftObject extends Lab9ObjectsKK {
	public ShaftObject() {
...
class MotorObject extends Lab9ObjectsKK {
	public MotorObject() {
...
class BladesObject extends Lab9ObjectsKK {
	...
	public TransformGroup position_Object() {              			// attach object BranchGroup "FanBlades" to 'objTG'
		...
		add_Audio();												// add audio to the rotation
	...
	private void add_Audio() {
		URL url = null;
		String name = "desk_fan";
		String filename = "src/codesKK280/sounds/" + name + ".wav";
		try {
			url = new URL("file","localhost", filename);			// load .wav file
		} catch (Exception e) {
			System.out.println("Can't open " + name);
		}
		
		// modify imported .wav file
		bgs = new BackgroundSound();
		bgs.setCapability(BackgroundSound.ALLOW_ENABLE_WRITE);
		bgs.setInitialGain(0.2f);
		BackgroundSoundBehavior player = new BackgroundSoundBehavior(bgs, url);
		player.setSchedulingBounds(CommonsKK.twentyBS);
		
		objRG.addChild(bgs);
	}
	public BackgroundSound get_BGS() {
		return bgs;
	}
}

class GuardObject extends Lab9ObjectsKK {
	public GuardObject() {
...
class StandObject extends Lab9ObjectsKK {
	public StandObject() {
...
class SwitchObject extends Lab9ObjectsKK {
	public SwitchObject() {
