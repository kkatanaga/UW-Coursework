...
class Plane extends Behavior {
	...
	private Transform3D plane_pos = null;
	...
	private float uv0[] = {0f, 0f};
	private float uv1[] = {1f, 0f};
	private float uv2[] = {1f, 1f};
	private float uv3[] = {0f, 1f};
	
	private TransformGroup createSquare(float s, Color3f color, float z) {
		Point3f[] verts = {new Point3f(s, -s, 0f), new Point3f(s, s, 0f), new Point3f(-s, s, 0f), new Point3f(-s, -s, 0f)};
		QuadArray qa = new QuadArray(4, GeometryArray.COORDINATES | GeometryArray.COLOR_3 | GeometryArray.TEXTURE_COORDINATE_2);
		...
		Transform3D radarTrfm = new Transform3D();
		trfm = new Transform3D();
		radarTrfm.rotY(Math.PI);
		trfm.setTranslation(new Vector3f(-1.6f, -0.05f, z));
		radarTrfm.mul(trfm);
		
		TransformGroup squareTG = new TransformGroup(radarTrfm);
		squareTG.addChild(new Shape3D(qa, new Appearance()));
		return squareTG;
	}
	public void addRadar() {
		rootTG.addChild(createSquare(0.35f, DFCommons.Black, 0f));
		
		pointTG = createSquare(0.011f, DFCommons.White, 0.01f);
		pointTG.setCapability(TransformGroup.ALLOW_TRANSFORM_READ);
		pointTG.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
		rootTG.addChild(pointTG);
	}
	...
	public void processStimulus(Iterator<WakeupCriterion> arg0) {
		...
		DogFight.viewTG.getTransform(trfm);
		trfm.mul(trfmStep);
		DogFight.viewTG.setTransform(trfm);
		
		enemy.getRootTG().getTransform(trfmStep);
		Vector3d planeVector = new Vector3d();
		Vector3d enemyVector = new Vector3d();
		trfm.get(planeVector);						// own position
		trfmStep.get(enemyVector);					// enemy position
		planeVector.sub(enemyVector);
		trfm.set(new Vector3d(planeVector.x * 0.00005, planeVector.z * 0.00005, 0));
//		
		pointTG.getTransform(trfmStep);
		trfm.mul(trfmStep);
		pointTG.setTransform(trfm);
	...
	private void reset_First_Bullet() {
		...
		plane_pos = new Transform3D();
		DogFight.viewTG.getTransform(plane_pos);
		bullet_temp.reset_Position(plane_pos);
		unusedBullets.add(bullet_temp);
